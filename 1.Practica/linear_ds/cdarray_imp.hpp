...
template <class T> size_t CDArray<T>::capacity() const { return capacity_; }

template <class T> size_t CDArray<T>::size() const { return size_; }

template <class T> bool CDArray<T>::is_empty() const { return size_ == 0; }

template <class T> bool CDArray<T>::is_full() const {
  return size_ == capacity_;
}

template <class T> CDArray<T>::CDArray(size_t cap) : capacity_(cap), size_(0) {
  assert(cap > 0);
...
size_t cInc(size_t v, size_t size) { return (v + 1) % size; }
...
template <class T> void CDArray<T>::grow() {
  ...
  // Actualizar el puntero del arreglo para que apunte al nuevo bloque de
  // memoria
  data_ = new_data;
