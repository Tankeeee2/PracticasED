      ...
      if (input_file) {
        if (line != "" && line[0] != '#') {
          ...
          if (command == "CREATE_LEAF") {
            ...
            if (input_line) {
              std::cout << "Creating leaf node " << idx << " [" << value << ']'
                        << std::endl;
            ...
            if (input_line) {
              std::cout << "Created node " << idx << " [" << value
                        << "] left_idx:" << left_idx
                        << " right_idx:" << right_idx << std::endl;
              nodes[idx] = BTNode<char>::create(value, nodes[left_idx],
                                                nodes[right_idx]);
            } else {
              std::cerr << "Error: wrong input format of test file."
                        << std::endl;
            ...
            input_line >> idx;
            if (input_line) {
              std::cout << "Node " << idx << " item: '" << nodes[idx]->item()
                        << "'." << std::endl;
            } else {
              std::cerr << "Error: wrong input format of test file."
                        << std::endl;
