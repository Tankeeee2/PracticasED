...
int main(int argc, const char *argv[]) {
  ...
  try {
    ...
    auto tree = std::make_shared<BTree<int>>();
    std::string command;
    ...
    while (input_file) {
      ...
      if (input_file) {
        if (command == "IS_EMPTY") {
          ...
          try {
            tree = std::make_shared<BTree<int>>(input);
          ...
          input_file >> v;
          if (!input_file) {
            std::cerr << "Error: input test file worng. " << std::endl;
            return EXIT_FAILURE;
          }
          size_t count = 0;
          std::cout << "Has (prefix search): " << v << "?: "
                    << (search_prefix<int>(*tree, v, count) ? 'Y' : 'N');
          std::cout << ". Processed " << count << " nodes." << std::endl;
          ...
          std::cout << "Is value " << v << " in ordered tree? "
                    << (has_in_order<int>(*tree, v) ? 'Y' : 'N') << std::endl;
