...
bool operator==(const Pattern &a, const Pattern &b) {
  return a.class_label() == b.class_label() && a.dim() == b.dim() &&
 ...
 */
static BTree<Pattern> create_kdtree(std::vector<Pattern>::iterator begin,
  ...
  if (size > 0) {
    ...
    // the begin and end iterators to set the range of the patterns for each
    // subtree. Remember: increment the level parameter.
...
KDTree::KDTree(std::istream &in) noexcept(false) : KDTree() {
  // TODO
...
std::tuple<float, Pattern>
KDTree::find_nn(Pattern const &p,
                const KDTree::distance_function_t &dist) const {
  assert(!is_empty());
  assert(p.dim() == get_k());
