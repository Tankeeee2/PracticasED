 ...
 * @tparam Comp is the comparison function object. Return true if a is before in
 * order to b.
 */
...
public:
   ...
   * For instance, std::greater_equal<T>() could be used if the queue give
   predominance to the
   ...
   *
   */
  PriorityQueue(std::vector<T> &values,
                Comp const &comp = std::greater_equal<T>());
