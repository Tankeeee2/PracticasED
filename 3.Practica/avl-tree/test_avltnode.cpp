  ...
  try {
    ...
    while (std::getline(input_file, line)) {
      ...
      if (!(input_line >> command)) {
        std::cerr << "Error: wrong format of test file." << std::endl;
        return EXIT_FAILURE;
      }
      ...
      if (command == "NODE_CREATE") {
        ...
        input_line >> idx;
        input_line >> item;
        std::cout << "Creating node idx " << idx << " with item " << item
                  << std::endl;
        node[idx] = AVLTNode<int>::create(item);
      } else if (command == "NODE_ITEM") {
        int idx;
        input_line >> idx;
        std::cout << "Node idx " << idx << " item: " << node[idx]->item()
                  << std::endl;
      } else if (command == "NODE_HEIGHT") {
        int idx;
        input_line >> idx;
        std::cout << "Node idx " << idx << " height: " << node[idx]->height()
                  << std::endl;
      } else if (command == "NODE_PARENT") {
        int idx;
        input_line >> idx;
        std::cout << "Node idx " << idx
                  << " parent's item = " << node[idx]->parent()->item()
                  << std::endl;
      } else if (command == "NODE_LEFT_CHILD") {
        int idx;
        input_line >> idx;
        std::cout << "Node idx " << idx
                  << " left child's item = " << node[idx]->left()->item()
                  << std::endl;
      } else if (command == "NODE_RIGHT_CHILD") {
        int idx;
        input_line >> idx;
        std::cout << "Node idx " << idx
                  << " right child's item = " << node[idx]->right()->item()
                  << std::endl;
      } else if (command == "NODE_ATTACH_LEFT") {
        int c_idx, p_idx;
        input_line >> c_idx >> p_idx;
        std::cout << "Attaching node idx " << c_idx
                  << " as left child of node idx " << p_idx << std::endl;
        node[p_idx]->set_left(node[c_idx]);
      } else if (command == "NODE_ATTACH_RIGHT") {
        int c_idx, p_idx;
        input_line >> c_idx >> p_idx;
        std::cout << "Attaching node idx " << c_idx
                  << " as right child of node idx " << p_idx << std::endl;
        node[p_idx]->set_right(node[c_idx]);
      } else if (command == "NODE_DETTACH_LEFT") {
        int idx;
        input_line >> idx;
        std::cout << "Dettaching left child of node idx " << idx << std::endl;
        node[idx]->set_left(nullptr);
      } else if (command == "NODE_DETTACH_RIGHT") {
        int idx;
        input_line >> idx;
        std::cout << "Dettaching right child of node idx " << idx << std::endl;
